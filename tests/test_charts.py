...
def test_annual_reference_keeps_unknown_periods_and_observed_2026_window(reference):
    ...
    assert annual.loc[annual.Fecha.lt("2027-03-01"), "Progreso_2026"].isna().all()
    assert annual.loc[annual.Fecha.gt("2027-09-01"), "Progreso_2026"].isna().all()
...
def test_reference_preserves_month_day_in_leap_year(reference):
    ...
    assert pd.isna(leap.loc["2028-02-29", "Progreso_Mediano"])
...
def test_historical_backdrop_does_not_leak_2026_into_earlier_cutoffs():
    ...
    assert annual.Progreso_2026.isna().all()
    assert annual.attrs["campaigns"] == ""
