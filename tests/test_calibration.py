...
def saved_profile():
    return load_site_profile(DATA / "azul_2026.json")


def apply(frame=None, **kwargs):
    arguments = {
        "site": "Azul", "as_of": "2027-01-08",
...
def test_invalid_profile_cannot_change_outputs():
    ...
    with pytest.raises(ValueError, match="finitos"):
        apply_site_calibration(example_trajectory(), profile, site="Azul", as_of="2027-01-08")
...
def real_data():
    ...
    raw = pd.read_csv(DATA / "azul_2026_counts.csv")
...
def test_source_units_missing_replicates_and_irregular_intervals(real_data):
    ...
    assert raw.PLM2.sum() == pytest.approx(8224.0)
    _, intervals = calibration_intervals(trajectory, prepared)
    assert intervals.iloc[0]["Inicio_exclusivo"] == pd.Timestamp("2026-03-01")
    assert intervals.iloc[0]["Fecha"] == pd.Timestamp("2026-03-26")
    assert intervals.iloc[0]["Dias_intervalo"] == 25
    assert intervals.iloc[0]["Observado_PLM2"] == pytest.approx(2713.)
    assert intervals.loc[intervals.Fecha.eq("2026-07-16"), "Dias_intervalo"].iloc[0] == 29
    assert intervals.Dias_intervalo.min() == 11
    assert intervals.Dias_intervalo.max() == 29
    assert len(intervals) == 10
    assert intervals.EE_repeticiones_PLM2.isna().all()
    assert np.allclose(intervals.Sigma_ajuste_PLM2, 271.3)
...
def test_fit_matches_persisted_profile_and_does_not_mutate_network(real_data):
    ...
    profile, comparison = fit_site_calibration(trajectory, prepared, site="Azul")
    saved = saved_profile()
...
def test_invalid_training_data_are_rejected(real_data, fault):
    ...
    with pytest.raises(ValueError):
        fit_site_calibration(trajectory, obs, site="Azul")
